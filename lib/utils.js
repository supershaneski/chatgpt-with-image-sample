...
export function formatText(text, bflag = false) {

    const flag = true // force
...
export function parseMarkdownImageLink(text) {
    // Regex to match the markdown image link syntax
    const regex = /!\[(.*?)\]\((.*?)\)/g;
  
    // Match the text and URL from the text
    ...
    // Return the text and URL as an array
